...
### 目标
1. **座位未满时**：玩家进入房间自动分配座位并自动准备，同时可点击"我要观战"切换为观战状态
...
**功能描述**：修改 `JoinRoom` / `AutoMatchAndJoin` 逻辑，当房间有空座位时，用户入房后自动分配一个空座位，状态为"已入座已准备"，同时保留手动切换为纯观战的入口。

**技术方案**：
- 修改 `JoinRoom` 应用服务方法，在执行 `LuaJoinAsSpectator` 后，判断是否有空座位
- 若有空座位，自动调用 `SelectSeat` 逻辑分配最低编号的空座位，并自动标记为"已准备"状态
- 新增 Lua 脚本 `LuaJoinAndAutoSeat`，将"加入房间+自动选座+自动准备"合并为原子操作，避免并发问题
- 复用现有 WebSocket 命令 `cancel_seat`（我要观战），允许已入座用户放弃座位变为纯观战者
- 前端需在入座状态下展示"我要观战"按钮（调用 `cancel_seat` 命令）
...
Client → WebSocket(join_room) → Gateway → gRPC → GameService.JoinRoom
  → LuaJoinAndAutoSeat:
      if 空座位存在 and 余额充足:
          加入观战者 + 自动分配最低编号空座位 + 自动准备 → 返回已入座已准备状态
...
### 我要观战流程
```
Client → WebSocket(cancel_seat) → Gateway → gRPC → SeatService.CancelSeat
  → LuaCancelSeat (复用现有):
      释放座位 → 检查排队队列 → 触发自动替补
  → 广播房间状态
```
...
| | 1.3 前端适配（展示"我要观战"按钮，调用 cancel_seat） | 0.5人天 |
| **模块2** | 满座自动观战 | 0.5人天 |
