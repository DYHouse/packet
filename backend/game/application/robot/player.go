...
func (p *RobotPlayer) JoinAndReady(ctx context.Context, roomID string, robotUserID string) error {
	_, err := p.roomAppService.JoinRoom(ctx, &application.JoinRoomRequest{
		RoomID: roomID,
		UserID: robotUserID,
	})
	if err != nil {
...
func (p *RobotPlayer) SelectSeat(ctx context.Context, roomID string, robotUserID string) error {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		_, err = p.seatAppService.SelectSeat(ctx, &application.SelectSeatRequest{
			RoomID: roomID,
			UserID: robotUserID,
			SeatNo: seatNo,
		})
...
func (p *RobotPlayer) Ready(ctx context.Context, roomID string, robotUserID string) error {
	_, err := p.seatAppService.PlayerReady(ctx, &application.PlayerReadyRequest{
		RoomID: roomID,
		UserID: robotUserID,
	})
	return err
...
func (p *RobotPlayer) SendPacket(ctx context.Context, roomID string, robotUserID string) error {
	_, err := p.gameAppService.SendPacket(ctx, &application.SendPacketRequest{
		RoomID: roomID,
		UserID: robotUserID,
	})
	return err
...
func (p *RobotPlayer) LeaveRoom(ctx context.Context, roomID string, robotUserID string) error {
	_, _ = p.seatAppService.CancelSeat(ctx, &application.CancelSeatRequest{
		RoomID: roomID,
		UserID: robotUserID,
	})
	_, err := p.roomAppService.LeaveRoom(ctx, &application.LeaveRoomRequest{
		RoomID: roomID,
		UserID: robotUserID,
	})
	if err != nil {
...
func (p *RobotPlayer) hasEmptySeat(state *repository.RoomStateData) bool {
	if state == nil {
		return false
	}
...
func (p *RobotPlayer) pickRandomEmptySeat(state *repository.RoomStateData) int {
	if state == nil {
		return 0
	}
	maxPlayers := state.MaxPlayers
