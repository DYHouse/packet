...
// 接口层已切换为 domain.RoundSettlement 聚合根，实现层在方法边界完成 domain ↔ model 转换。
type RoundSettlementRepository interface {
	...
	GetRoundSettlementByRoundID(ctx context.Context, roundID int64) (*domain.RoundSettlement, error)
	...
	CreateRoundSettlementAndBills(ctx context.Context, settlement *domain.RoundSettlement, bills []*domain.BillRecord) error
	...
	GetFailedFirstRoundSettlements(ctx context.Context, since time.Time, limit int) ([]*domain.RoundSettlement, error)
	// GetDeductedButNotSettled 查询已扣款但未结算的记录（用于异常检测）
	GetDeductedButNotSettled(ctx context.Context, since time.Time, limit int) ([]*domain.RoundSettlement, error)
	// GetAllRoundSettlementsBySession 查询该会话所有回合的结算记录（按 round_no 正序）
	GetAllRoundSettlementsBySession(ctx context.Context, sessionID int64) ([]*domain.RoundSettlement, error)
